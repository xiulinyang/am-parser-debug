...
def forward_on_instances(model,
    ...
    with torch.no_grad():
        ...
        for dataset in data_iterator._create_batches(instances, shuffle=False):
            ...
            if "@@UNKNOWN@@" not in model.vocab.get_token_to_index_vocabulary (namespace='DRS_supertag_labels'):
                model.vocab.add_token_to_namespace ("@@UNKNOWN@@", namespace='DRS_supertag_labels')
            print(model.vocab)
