...
#============TRAINING PARAMETERS==============
local batch_size = 10; # adjust this to fit the memory capacity of your machine (mostly GPU memory is relevant)
local num_epochs = 200; # we recommend 40-100 epochs on real datasets and 200 epochs for toy datasets, but your optimal parameters may differ
local patience = 100; # this is per default set to 1000 to turn off early stopping.
local evaluate_on_test = false; # Whether to evaluate on the test set.


#============FILEPATHS==============
local train_zip_path = "./output_exp/train.zip";  
local dev_zip_path = "./output_exp/dev.zip";
local validation_amconll_path = "./output_exp/drt_dev.amconll";
local validation_gold_path = "./output_exp/gold_dev.txt";
local test_triples_amconll_gold_suffix = [["./output/drt_eval.amconll", "./output/gold_eval.txt", ""]];  # each triple consists of (a) path to amconll input for test set; (b) path to gold output for test set; (c) a suffix to be attached to the names of the evaluation metrics. Use multiple triples to evaluate on multiple test sets. In the regular case of just using one test set, use just one triple and the suffix can be the empty string.


#=============IMPORTING MODEL AND FORMALISM CONFIGS==================
local formalism_config = import '../formalisms/drs.libsonnet';
