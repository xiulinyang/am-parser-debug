...
#============TRAINING PARAMETERS==============
local batch_size = 48; # adjust this to fit the memory capacity of your machine (mostly GPU memory is relevant)
...
local patience = 20; # this is per default set to 1000 to turn off early stopping.
local evaluate_on_test = false; # Whether to evaluate on the test set.


#============FILEPATHS==============
local train_zip_path = "./multibox/train.zip";  
local dev_zip_path = "./multibox/dev.zip";
local validation_amconll_path = "./multibox/drt_dev.amconll";
local validation_gold_path = "./multibox/gold_multibox_dev5.txt";
local test_triples_amconll_gold_suffix = [["./scopeless5/drt_eval.amconll", "./scopeless5/gold_eval.txt", ""]];  # each triple consists of (a) path to amconll input for test set; (b) path to gold output for test set; (c) a suffix to be attached to the names of the evaluation metrics. Use multiple triples to evaluate on multiple test sets. In the regular case of just using one test set, use just one triple and the suffix can be the empty string.


#=============IMPORTING MODEL AND FORMALISM CONFIGS==================
local formalism_config = import '../formalisms/drs_scopeless_exp1.libsonnet';

local raw_model_config = import '../models/finetune2021.libsonnet';
