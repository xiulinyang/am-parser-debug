...
#============FILEPATHS==============
local train_zip_path = "./output_simplifiedSBN/train.zip";  
local dev_zip_path = "./output_simplifiedSBN/dev.zip";
local validation_amconll_path = "./output_simplifiedSBN/drt_dev.amconll";
local validation_gold_path = "./output_simplifiedSBN/gold_dev.txt";
local test_triples_amconll_gold_suffix = [["./output_simplifiedSBN/drt_eval.amconll", "./output_simplifiedSBN/gold_eval.txt", ""]];  # each triple consists of (a) path to amconll input for test set; (b) path to gold output for test set; (c) a suffix to be attached to the names of the evaluation metrics. Use multiple triples to evaluate on multiple test sets. In the regular case of just using one test set, use just one triple and the suffix can be the empty string.


#=============IMPORTING MODEL AND FORMALISM CONFIGS==================
local formalism_config = import '../formalisms/drs_simplifiedSBN.libsonnet';
