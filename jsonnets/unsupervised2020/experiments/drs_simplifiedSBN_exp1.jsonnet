...
#============TRAINING PARAMETERS==============
local batch_size = 64; # adjust this to fit the memory capacity of your machine (mostly GPU memory is relevant)
...
#============FILEPATHS==============
local train_zip_path = "./simplifiedSBN_exp1/train.zip";  
local dev_zip_path = "./simplifiedSBN_exp1/dev.zip";
local validation_amconll_path = "./simplifiedSBN_exp1/drt_dev.amconll";
local validation_gold_path = "./simplifiedSBN_exp1/gold_dev.txt";
local test_triples_amconll_gold_suffix = [["./scopeless_exp1/drt_eval.amconll", "./scopeless_exp1/gold_eval.txt", ""]];  # each triple consists of (a) path to amconll input for test set; (b) path to gold output for test set; (c) a suffix to be attached to the names of the evaluation metrics. Use multiple triples to evaluate on multiple test sets. In the regular case of just using one test set, use just one triple and the suffix can be the empty string.
